...
Hooks.on("pf2e.useAction", async function (message, actor, item, outcome) {
    const actionData = {
        type: "actionUse",
        actorId: actor.id,
        itemData: item.system,
    ...
    await sendActionData(actionData);
    if (window.sendCharacterStatsForActor) {
        await window.sendCharacterStatsForActor(item.actor);
    }
});
...
async function sendActionData(actionData) {
    game.socket.emit("module.foundryvtt-to-nodecg", {
        type: "actionEvent",
        data: actionData
    });
    console.log("Action data sent to external system:", actionData);
}